...
func (v MoviesResource) Show(c buffalo.Context) error {
	...
	ImdbData := getMovieData(ImdbIDString(movie.Imdb))
...
func getMovieData(imdbID string) models.ImdbData {
	data := models.ImdbData{}
	var omdbAPIKey = envy.Get("OMDB_API_KEY", "nothing")
	log.Printf("%+v\n", omdbAPIKey)

	s := []string{"http://www.omdbapi.com/?i=", imdbID, "&apikey=", omdbAPIKey}
